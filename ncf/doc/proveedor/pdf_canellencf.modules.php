...
/**
 *	\file       htdocs/core/modules/supplier_invoice/doc/pdf_canellencf.modules.php
 *	\ingroup    fournisseur
 ...
 */
class pdf_canellencf extends ModelePDFSuppliersInvoices
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "canellecnf";
		$this->description = 'Plantilla completa de Factura de proveedor NCF';
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->fournisseur->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				// Display info area
				$posy = $this->_tableau_info($pdf, $object, $bottomlasttab, $outputlangs, $deja_regle);
	...
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs, $deja_regle)
	{
		// phpcs:enable
		global $conf, $mysoc;
		$data = Array();
		...
		$data['total_ht'] = $sign * ($total_ht + (!empty($object->remise) ? $object->remise : 0));
		...
		foreach ($this->tva as $tvakey => $tvaval)
		{
			if ($tvakey > 0)    // We do not display rate 0
			{
				...
				$data['TotalVAT'] = $tvaval;
			}
		}
		if (!$this->atleastoneratenotnull) // If no vat at all
		{
			$data['TotalVAT'] = $object->total_tva;
		}

		// Total TTC
		$data['TotalTTC'] = $sign * $total_ttc;
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS))
		{
			...
			$data['Paid'] = price($deja_regle + $depositsamount, 0, $outputlangs);

			// Credit note
			if ($creditnoteamount)
			{
				$data['CreditNotes'] = price($creditnoteamount, 0, $outputlangs);
			}

			// Escompte
			if ($object->close_code == FactureFournisseur::CLOSECODE_DISCOUNTVAT)
			{
				$data['EscompteOfferedShort'] = price($object->total_ttc - $deja_regle - $creditnoteamount - $depositsamount, 0, $outputlangs);
				$resteapayer = 0;
			}

			$data['RemainderToPay'] = price($resteapayer, 0, $outputlangs);
		}

		require_once DOL_DOCUMENT_ROOT.'/ncf/lib/ncf.lib.php';
		$dataTemp = ComprobantesPdfIvaRetencion($object->id);

		$ivabaseTemp = '';
		$ivaItebisTemp = '';
		$ivaDecTemp = '';
		if (isset($data['TotalVAT'])){
			$ivaTemp = $data['TotalVAT'];

			if ($dataTemp != 0){
				$ivabaseTemp = ($dataTemp->base != 'no') ? ($data['total_ht']/ 100)*$dataTemp->base : 0;
				$ivaItebisTemp = ($dataTemp->itebis != 'no') ? ($ivaTemp / 100)*$dataTemp->itebis : 0;
		...
		$pdf->MultiCell(60, 4, 'Total pagado : '.$data['Paid'], 0, 'L');

		// if (isset($data['TotalVAT']) && $dataTemp != 0) {
		$deja_regle = $object->getSommePaiement((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? 1 : 0);
		$posy += 5;
		$data['total_ht'] = (isset($data['total_ht'])) ? ($ivaDecTemp + $data['total_ht']) : 0;
		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->MultiCell(60, 4, 'Total adeudado : '.price($data['TotalTTC'] - $deja_regle, 0, $outputlangs), 0, 'L');
		// }else{
		// 	$posy += 5;
		// 	$data['TotalTTC'] = (isset($data['TotalTTC'])) ? price($data['TotalTTC'], 0, $outputlangs) : 0;
		// 	$pdf->SetXY($this->marge_gauche, $posy);
		// 	$pdf->MultiCell(60, 4, 'Total adeudo : '.$data['TotalTTC'], 0, 'L');
		// }

		$posy += 5;
		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->MultiCell(60, 4, 'Total retenciones : '.price((float)$ivabaseTemp + (float)$ivaItebisTemp), 0, 'L');

		if (isset($data['TotalVAT'])){
			if ($dataTemp != 0) {

				if ($dataTemp->base != 'no' || $dataTemp->itebis != 'no') {
					$posy += 7;
					$pdf->SetFont('', 'B', $default_font_size);
					$pdf->SetXY($this->marge_gauche, $posy);
					$pdf->MultiCell(60, 4, 'Retenciones', 0, 'L');
				}
				...
				if ($dataTemp->base != 'no'){
					...
					$pdf->SetFont('', '', $default_font_size);
					$pdf->SetXY($this->marge_gauche, $posy);
					$pdf->MultiCell(80, 4, 'Retención sobre base imponible : '.$ivabaseTemp ."\n".$dataTemp->baselabel, 0, 'L');
				}

				if ($dataTemp->itebis != 'no'){
					$posy += 10;
					$pdf->SetFont('', '', $default_font_size);
					$pdf->SetXY($this->marge_gauche, $posy);
					$pdf->MultiCell(80, 4, 'Retención sobre impuestos : '.$ivaItebisTemp ."\n".$dataTemp->itebislabel, 0, 'L');
				}
			}
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		foreach ($this->tva as $tvakey => $tvaval)
		{
			if ($tvakey > 0)    // We do not display rate 0
			{
				...
				$ivaTemp = $tvaval;
				$valTempcol2x = $col2x;
				$valTempcol2x_2= $tab2_top + $tab2_hl * $index;
				$valTemplargcol2 = $largcol2;
				$valTemptab2_hl = $tab2_hl;
				$ivaIS = 1;
			}
		}
		...
		//
		// if ($dataTemp != 0){
		// 	$ivabaseTemp = ($dataTemp->base != 'no') ? ($ivaTemp/ 100)*$dataTemp->base : 0;
		// 	$ivaItebisTemp = ($dataTemp->itebis != 'no') ? ($ivaTemp / 100)*$dataTemp->itebis : 0;
		//
		// 	$ivabaseTemp = round($ivabaseTemp, 2);
    //     	$ivaItebisTemp = round($ivaItebisTemp, 2);
		//
		// 	if (!(($ivaTemp - $ivabaseTemp) >= $ivaItebisTemp)){
		// 		$ivaItebisTemp = $ivaTemp - $ivabaseTemp;
		// 	}
		// }
		...
		// 		$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
		// 		$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Retención sobre base imponible', $useborder, 'L', 0);
		//
		// 		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		// 		$pdf->MultiCell($largcol2, $tab2_hl, '-'.$ivabaseTemp, $useborder, 'R', 0);
		// 		$pdf->SetFont('', '', $default_font_size - 1);
		// 		$pdf->SetTextColor(0, 0, 0);
		// 	}
		//
		...
		// 		$pdf->SetFillColor(224, 224, 224);
		// 		$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
		// 		$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Retención sobre impuestos', $useborder, 'L', 0);
		//
		// 		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		// 		$pdf->MultiCell($largcol2, $tab2_hl, '-'.$ivaItebisTemp, $useborder, 'R', 0);
		// 		$pdf->SetFont('', '', $default_font_size - 1);
		// 		$pdf->SetTextColor(0, 0, 0);
		// 	}
		// }
		...
		//if ($ivaIS && $dataTemp != 0){
		//	$pdf->SetXY($valTempcol2x, $valTempcol2x_2);
		//	$pdf->MultiCell($valTemplargcol2, $valTemptab2_hl, $ivaDecTemp, 0, 'R', 0);
		//}else if ($ivaIS){
			$pdf->SetXY($valTempcol2x, $valTempcol2x_2);
			$pdf->MultiCell($valTemplargcol2, $valTemptab2_hl,  price($ivaTemp, 0, $outputlangs), 0, 'R', 0);
		//}
		...
		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $total_ht + $object->total_tva, 0, $outputlangs), $useborder, 'R', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		require_once DOL_DOCUMENT_ROOT.'/ncf/lib/ncf.lib.php';
		$dataTemp = ComprobantesPdfHead($object->id);
		...
		if ($dataTemp->fven != '---' && $dataTemp != 0) {
			$posy += 4;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell(100, 3, 'Fecha de vencimiento : '.date("d/m/Y", $object->date_echeance), '', 'R');
		}
		...
		if ($dataTemp->label != '---' && $dataTemp != 0) {
			$posy += 4;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell(100, 3, 'Tipo de comprobante: '.$dataTemp->label, '', 'R');
		}

		if ($dataTemp->sec != '---' && $dataTemp != 0) {
			$posy += 4;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell(100, 3, 'Ncf : '.$dataTemp->sec, '', 'R');
		}
