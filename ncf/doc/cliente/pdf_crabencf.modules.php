...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_crabencf.modules.php
 *	\ingroup    facture
 ...
 */
class pdf_crabencf extends ModelePDFFactures
{
	 /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "crabencf";
		$this->description = 'Factura PDF plantilla Crabe Ncf';
	...
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		// $amount_credit_notes_included = $object->getSumCreditNotesUsed((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? 1 : 0);
		// $amount_deposits_included = $object->getSumDepositsUsed((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? 1 : 0);
		// $totalAdeudo = price2num($object->total_ttc - $deja_regle - $creditnoteamount - $depositsamount, 'MT');
		$deja_regle = $object->getSommePaiement((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? 1 : 0);
		$depositsamount = $object->getSumDepositsUsed((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? 1 : 0);
		$ivabaseTemp = '';
		$ivaItebisTemp = '';
		...
		if ($dataTemp != 0){
			$ivabaseTemp = ($dataTemp->base != 'no') ? ($object->total_ht/ 100)*$dataTemp->base : 0;
			$ivaItebisTemp = ($dataTemp->itebis != 'no') ? ($object->total_tva / 100)*$dataTemp->itebis : 0;
		...
		// 	$pdf->SetXY($this->marge_gauche, $posy);
		// 	$pdf->MultiCell(60, 4, 'Total adeudo : '.price($object->total_ttc), 0, 'L');
		// }
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull)
			{
				...
				// 	// 	$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				// 	// 	$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'R. base', $useborder, 'L', 0);
				// 	//
				// 	// 	$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				...
				// 		$pdf->SetFillColor(224, 224, 224);
				// 		$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				// 		$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'R. ITEBIS', $useborder, 'L', 0);
				//
				// 		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				...
				//	$pdf->MultiCell($valTemplargcol2, $valTemptab2_hl, $ivaTemp, 0, 'R', 0);
				//}else if ($ivaIS){
					$pdf->SetXY($valTempcol2x, $valTempcol2x_2);
					$pdf->MultiCell($valTemplargcol2, $valTemptab2_hl, price(price2num($ivaTemp, 'MT'), 0, $outputlangs), 0, 'R', 0);
				//}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// if ($dataTemp->label != '---' && $dataTemp != 0) {
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, 'Tipo de comprobante: '.$dataTemp->label, '', 'R');
		// }
		...
		if ($dataTemp->sec != '---' && $dataTemp != 0) {
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, 'Ncf : '.$dataTemp->sec, '', 'R');
		}
